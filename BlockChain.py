...
class Blockchain(object):
    ...
    def newBlock(self, proof, previousHash = None):
        block={
            ...
            'proof' :proof,
            'previousHash' :previousHash or self.hash(self.chain[-1]), #اخرین بلاک را یه شکل جیسون در آورده و هش میکند
        }
        self.currentTrxs = [] #تمام مقادیر را پاک میکنیم تا دوباره ترنزکشن های جدید با بلاک بعدی ساخته شوند
    ...
    ''' use for set block previous hash '''
    @staticmethod
    def hash(block):
    ...
    def validProof(lastProof, proof, trxs):
        thisProof = f'{proof}{lastProof}{trxs}'.encode() #تبدیل به رشته میکنیم
        thisProofHash = hashlib.sha256(thisProof).hexdigest()
        return thisProofHash[:4] == '0000' #اگر اشتباه باشد تایع باز فراخانی میشود با یک بروف بالاتر
    ...
    def validChain(self, chain):
        ...
        while currentIndex < len(chain):
            ...
            if(block['previousHash'] != self.hash(lastBlock)): # چی میکند هش بلاک قبلی که در این بلاک قرار دارد درست است
                return False

            if not self.validProof(lastBlock['proof'], block['proof'], block['trxs']): # ابتدای هش کد 0000 است یا نه
                return False
            
            lastBlock = block
            currentIndex += 1
        
        return True #اگر تمام پیمایش انجام شود و مشکلی در هش ها وجود نداشته باشد 
...
# test = Blockchain
# test.newTrx("mohammad" , "ali" , "2")
# test.newTrx("ali" , "reza" , "1")
